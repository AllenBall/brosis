...
**采集适配规则。** 不同应用的界面结构差别很大，逐个写规则：Safari、Claude 桌面版、ChatGPT、飞书、微信、Telegram 各有专门规则，其余走通用规则。规则描述"正文在哪个区域、怎么读、读不到时退回什么"。

Telegram for macOS（原生版）的无障碍树对内容是死的：窗口报一百多个子节点，整体拷贝失败，按下标只拿回两根滚动条和几个按钮，一个文本节点都没有。所以它和微信一样走视口 OCR（聊天面板 + 顶栏会话名，窗口定向截图），但分栏边界不靠写死的点数：先从那两根滚动条的 frame 现场算（字号、拖动分隔线、置顶条、窄窗单栏都跟着变），算不出才用 `adapter.telegram.sidebarWidth / titleBarHeight / composerHeight` 那组兜底值。群聊看会话头第二行「N members」判定，发送者名取气泡内首行；语音只显示成时长、认不成 [语音]；经典（非气泡）外观下左右归属不可靠；Telegram Desktop（Qt 版）没有适配。微信与 Telegram 的窗口标题都是恒定的，AX 那条路的观察改用上一次从标题条认出的会话名当标题；AX 读焦点窗口超时时，这两条纯 OCR 规则改用系统窗口表里的窗口矩形照常 OCR，不再整帧丢掉。

ChatGPT 桌面版（26.9 起是 Codex 应用，内核是 Chromium 而不是 Electron）的无障碍树是「戳一下才建」的：不认 `AXManualAccessibility`，网页视图在无障碍模式没开时根本不出现在树里，只有命中测试再读一次角色才会让 Chromium 把树建起来；隐藏五分钟后树又会被撤掉。所以 brosis 读到空树时先戳再重扫，戳不起来才回退 OCR。建好的树里正文在第 17–21 层，通用规则的 12 层限额一个字都读不到——通用 Chromium 规则的限额随之放到 40 层。专属规则只读主区 `<main>`：侧栏的会话列表 / 项目 / 导航不进正文，输入框草稿与输入区的模型 / 权限 / 项目选择器不记；截图走窗口定向；树没建时回退 OCR 的矩形按侧栏宽内缩（默认 276，`adapter.chatgpt.sidebarWidth` 可校准）。窗口标题恒为「ChatGPT」，会话名与消息行的归属（谁说的、代码块）尚未适配。
