...
**Adapter rules.** App UIs differ enough that each gets its own rule: Safari, Claude desktop, ChatGPT, Feishu/Lark, WeChat and Telegram have dedicated rules; everything else uses a generic one. A rule describes where the text lives, how to read it, and what to fall back to when that fails.

Telegram for macOS (the native app) has a dead accessibility tree as far as content goes: the window reports well over a hundred children, copying them fails as a whole, and per-index copies return only two scroll bars and a few buttons — not a single text node. So it goes the WeChat way (viewport OCR of the chat panel and the header, window-targeted capture), except that the pane bounds are not hard-coded: they are derived live from the frames of those two scroll bars (they follow font size, a dragged divider, the pinned-message bar and the narrow single-column layout), with `adapter.telegram.sidebarWidth / titleBarHeight / composerHeight` only as the last fallback. Group chats are recognised from the header's second line ("N members"), the sender name is the first line inside the bubble; voice messages show only a duration and are not tagged as voice; the classic (non-bubble) look breaks left/right attribution; Telegram Desktop (the Qt build) is not covered. WeChat and Telegram both have a constant window title, so AX-path observations now reuse the last conversation name read from the header; and when the AX focused-window read times out, these two OCR-only rules fall back to the window rectangle from the system window list instead of dropping the frame.

The ChatGPT desktop app (the Codex app since 26.9, built on Chromium rather than Electron) has an accessibility tree that only gets built when poked: it does not honour `AXManualAccessibility`, the web view is simply absent from the tree while accessibility mode is off, and only a hit-test followed by a role read makes Chromium build it; five minutes after the window is hidden the tree is torn down again. So when brosis reads an empty tree it pokes first, then rescans, and only falls back to OCR if the tree still does not appear. Once built, the text sits 17–21 levels deep, where the generic rule's 12-level limit reads nothing at all — the generic Chromium rule now allows 40 levels. The dedicated rule reads only the `<main>` landmark: the sidebar's conversation list, projects and navigation stay out of the body, the composer draft and its model / permission / project selectors are not recorded; capture is window-targeted; when the tree is not built, the fallback OCR rectangle is inset by the sidebar width (default 276, tunable via `adapter.chatgpt.sidebarWidth`). The window title is always "ChatGPT"; conversation names and per-message attribution (who said what, code blocks) are not adapted yet.
