...
**Adapter rules.** App UIs differ enough that each gets its own rule: Safari, Claude desktop, Feishu/Lark, WeChat and Telegram have dedicated rules; everything else uses a generic one. A rule describes where the text lives, how to read it, and what to fall back to when that fails.
