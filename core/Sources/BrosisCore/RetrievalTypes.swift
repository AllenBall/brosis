...
public struct RetrievalOptions: Sendable {
    ...
    /// **间隔判据**（e 批 ⑧a）：最好的那一块，要比候选大盘好出这么多，向量通道才算有话说。
    /// `0` = 关掉，行为与只有绝对阈值时逐位相同。
    ///
    /// 为什么光有绝对阈值不够：`vectorMaxDistance` 的已知毛病是**不随索引规模自适应**——
    /// 同一套题、同一个阈值，索引从 37% 建到 66% 时负例误报就从 0 涨到 2。原因是块越多，
    /// 「库里根本没有的内容」的最近邻也越近，迟早会挤进任何固定的绝对阈值里。
    ///
    /// 相对阈值（按最好的那条放宽）救不了这个：误报场景里所有候选都聚成一团，
    /// 相对规则照样全部放行。有区分力的是**分布形状**：真答案会从大盘里凸出来，
    /// 而"库里没有"时候选彼此差不多。所以判据是
    /// `median(候选距离) - min(候选距离) >= vectorMinSeparation`，不满足就整条通道不出声
    /// （`vectorUnavailableReason = "no_separation"`），而不是硬挑一批出来。
    ///
    /// **实测结论（2026-09-09，46,545 块、47 道改写题 + 60 道原题）：这个判据不成立，默认关。**
    ///
    ///     间隔    改写题召回  救回  原 60 题误报
    ///     0       0.387       39    2
    ///     0.03    0.387       39    2      ← 完全没生效
    ///     0.05    0.378       38    2
    ///     0.08    0.343       33    2
    ///     0.12    0.196       19    1      ← 召回腰斩才换来少 1 条误报
    ///
    /// 误报数从头到尾卡在 2，直到把召回砍掉一半才掉到 1。说明那 2 条**不是**"候选聚成一团、
    /// 矮子里挑高个"造成的，而是确实有一个错的文本版本在语义上离查询很近——
    /// 分布形状区分不了这个，只有内容本身能。
    ///
    /// 旋钮和 `tools/eval/d8_separation_sweep.py` 都留着：它们是得出这个结论的量具，
    /// 换语料（真实查询题 D12）或换模型（4B / 8B）时可以再量一次，不必重新想一遍。
    /// **但别在没重跑扫描的情况下把它打开。**
    ///
    /// 「阈值不随索引规模自适应」这个原问题**仍然没解决**。这次的数据还给出了下一步该测什么：
    /// 误报由绝对阈值决定（今早的扫描里 0.50 → 0.55 让误报 2 → 8），与分布形状无关，
    /// 所以要验的是"索引从 37% 长到 100% 时同一阈值的误报曲线"——那要按不同索引规模
    /// 各建一次库，比这次贵得多。
    public var vectorMinSeparation: Double = 0
